...
def Detection(net,image):
    ...
    for out in outs:
        for detection in out:
            ...
            if confidence > 0.6: # only when high enough confidence
    ...
    for i in indices:
        i = i[0]
        box = boxes[i]    # numbers are [center_x, center_y, width, height]
        left = box[0]
        top = box[1]
        width = box[2]
        height = box[3]
        cv2.rectangle(image, (left, top), (left + width, top + height), (0, 0, 255), 2, 8, 0)
        cv2.putText(image, classes[classIds[i]] + str(round(confidences[i], 2)), (left, top),cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 0), 2)
...
if __name__ == '__main__':
    ...
    with open(model_classname, 'rt') as f: #coco.names
    ...
    while True:
        ...
        Obj_img = Detection(net,rs_config.D435_para.colormat) #Gather and paint information
        for Obj in Objection_vec:
            # print(Obj.classname, ":", Obj.PCL)
            Position='x:%d y:%d z:%d' %(Obj.PCL[0],Obj.PCL[1],Obj.PCL[2])
            SpaceDiagonal=str(int(math.sqrt((Obj.PCL[0])*(Obj.PCL[0]) + (Obj.PCL[1])*(Obj.PCL[1]) + (Obj.PCL[2]) * (Obj.PCL[2]))))
            # print(Obj.Point[1])
            cv2.putText(Obj_img, Position, (int(Obj.Point[1]-50), int(Obj.Point[0])-50), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 255, 0))
            cv2.putText(Obj_img, 'Distance: ' + SpaceDiagonal, (int(Obj.Point[1]), int(Obj.Point[0])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0))
        
        cv2.imshow('RealSenseRGB',Obj_img)
        #cv2.imshow('RealSenseDepth',rs_config.D435_para.depthmat)
        
        #Make depthmat-image brighter
        cv2.imshow('RealSenseDepthBrighter', rs_config.D435_para.depthmat*5)
